...
test_that("Check cycle counts from a complete graph with n = 10.",
          {
            ...
            expect_equal(as.vector(.count_k_cycle(igraph::as_adjacency_matrix(G,sparse= FALSE), 7)), 
...
test_that("Check cycle counts from a subset of political blog dataset",
          {
            G <- igraph::as_adjacency_matrix(igraph::upgrade_graph(polblog), sparse = FALSE)[1:50,1:50]
