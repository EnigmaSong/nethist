...
##' @param save.plot A logical indicating whether to save the generated figure. If `TRUE`, the plot is saved via [ggplot2::ggsave()] using the specified file name. Otherwise, the plot is displayed.
...
##' @returns
##' A network summary plot and a data frame of network summary statistics.
##' @details
##' Vertex sampling is done by simple random sampling without replacement.
##'
##' The following matrix classes are supported: [base::matrix], [Matrix::dgCMatrix-class]
...
##' netsummary_plot(A, R = 500, subsample_sizes = 150)
##'
...
##' \dontrun{
##' netsummary_plot(A, save.plot = TRUE, filename = "myfig.pdf")
##' }
##' }
...
netsummary_plot.igraph <- function(A,
  ...
  do.call("netsummary_plot.default", args = args)
}
...
netsummary_plot.default <- function(A,
  ...
  if (is.na(subsample_sizes)) {
    subsample_sizes <- auto_select_subsample_sizes(A, Ns, k_max = max_cycle_order,
                                                   R, alpha = 0.05, delta = 0.05)
  }
