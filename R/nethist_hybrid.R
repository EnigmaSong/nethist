...
##' \item `details` list of all candidate models, one entry per number of shapes from 1 up to the maximum considered, each containing `s`, `blockcluster`, `thetahat`, `MSE`, `normalized_LL`, and `BIC`.
...
.hnethist_matrix <- function(A, h = NA,
  ...
  s_max <- min(k * (k + 1) / 2.0,
              length(unique(
                result$initial$thetahat[
                  upper.tri(result$initial$thetahat, diag = TRUE)])))
  result$details <- vector(mode = "list", length = s_max)

  # Model selection
  for (ss in s_max:1) {
    result$details[[ss]]$n          <- n
    result$details[[ss]]$s          <- ss
    result$details[[ss]]$blockcluster <- hnethist_kmeans(result$initial,
                                                         centers = ss)
    result$details[[ss]]$thetahat    <- hnethist_smoothing(
                                          result$details[[ss]]$blockcluster, k)
    result$details[[ss]]$MSE        <- hnethist_LSE(
                                         result$details[[ss]]$thetahat,
                                         result$initial$cluster, A)
    ll <- hnethist_LL(result$details[[ss]]$thetahat,
                      result$initial$cluster, A)
    result$details[[ss]]$normalized_LL <- ll / sum(A)
    result$details[[ss]]$BIC        <- hnethist_BIC(result$details[[ss]], ll)
  }
