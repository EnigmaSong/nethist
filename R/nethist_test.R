...
##' @export
nethist_test<-function(A, h = NA, outfile, verbose = FALSE){
  UseMethod("nethist_test")
}
##' @exportS3Method 
nethist_test.igraph<-function(A, h, outfile, verbose){
  args <- as.list(environment())
  args$A <- igraph::as_adj(args$A, sparse = FALSE)
  do.call("nethist_test.default", args)
}
##' @exportS3Method 
nethist_test.matrix<-function(A, h, outfile, verbose){
  args <- as.list(environment())
  do.call("nethist_test.default", args)
}
##' @exportS3Method 
nethist_test.dgCMatrix<-function(A, h, outfile, verbose){
  args <- as.list(environment())
  args$A <- as.matrix(args$A)
  do.call("nethist_test.default", args)
}
##' 
nethist_test.default <- function(A, h = NA, outfile, verbose = F){
  ...
  idx <- .graphest_fastgreedy_test(A,h,idxInit, verbose)
